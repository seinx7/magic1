...
function generateFloatingCandles() {
    ...
    if(!container) return;
    const positions = [{l:'5%', t:'20%'}, {l:'92%', t:'15%'}, {l:'8%', t:'75%'}, {l:'90%', t:'65%'}, {l:'20%', t:'85%'}, {l:'80%', t:'82%'}];
    positions.forEach(pos => {
        const c = document.createElement('div');
        c.className = 'candle';
        c.style.left = pos.l; c.style.top = pos.t;
...
function updateUI() {
    ...
    if(me) {
        ...
        if(nameEl) nameEl.innerText = me.nickname + (me.isEliminated ? ' (아즈카반)' : '');
        if(houseEl) houseEl.innerText = getKRHouse(me.house);
        ...
        if(me.isEliminated) {
            document.querySelectorAll('.professor-card').forEach(c => c.style.opacity = '0.5');
        }
    }
}
...
function showMinistryNotice(msg, isError = false) {
    ...
    document.getElementById('notice-msg').style.color = isError ? '#8b0000' : '#2C241D';
    ...
    for(let id in gameState.players) {
        ...
        card.innerHTML = `
            <div style="display:flex; justify-content:space-between; align-items:center; color:#000;">
                ...
                <div>
                    <select onchange="adminAction('setHouse', '${id}', {house: this.value})"><option value="">기숙사</option><option value="Gryffindor" ${p.house==='Gryffindor'?'selected':''}>Gry</option><option value="Slytherin" ${p.house==='Slytherin'?'selected':''}>Sly</option><option value="Ravenclaw" ${p.house==='Ravenclaw'?'selected':''}>Rav</option><option value="Hufflepuff" ${p.house==='Hufflepuff'?'selected':''}>Huf</option></select>
