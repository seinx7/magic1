...
// --- 경로 자동 탐지 로직 ---
// 1. 현재 위치(__dirname)에 'public' 폴더가 있는지 확인
let publicPath = path.join(__dirname, 'public');

// 2. 만약 'public' 폴더가 없다면(이미 public 폴더 안에 server.js가 있는 경우)
if (!fs.existsSync(publicPath)) {
    publicPath = __dirname; // 현재 위치를 파일 저장소로 사용
}
...
app.get('/', (req, res) => {
    ...
    if (fs.existsSync(indexPath)) {
        ...
        res.status(404).send("지도를 찾을 수 없습니다. (index.html 파일이 보이지 않습니다!)");
    }
});
...
io.on('connection', (socket) => {
    ...
    socket.on('startMiniGame', (type) => {
        ...
        if (type === 'creature') {
            if (!gameState.garticLink) return socket.emit('error', '리쵸 교수님이 아직 문을 열지 않으셨습니다.');
...
server.listen(PORT, '0.0.0.0', () => {
    console.log(`Hogwarts Online is running on port ${PORT}`);
});
