...
io.on('connection', (socket) => {
    ...
    socket.on('join', (nickname) => {
        if (gameState.status !== 'waiting') return socket.emit('error', '이미 학기가 시작되었습니다.');
    ...
    socket.on('adminAction', (data) => {
        if (data.password !== ADMIN_PASSWORD) return socket.emit('error', '인가 주문이 틀렸습니다.');
        ...
        if (data.type === 'startGame') {
            ...
            for(let h in data.scores) { if(gameState.houses[h]) gameState.houses[h].score = parseInt(data.scores[h]) || 0; }
    ...
    socket.on('report', (targetNick) => {
        ...
        if (!targetEntry) return socket.emit('error', '그 마법사는 존재하지 않습니다.');
...
server.listen(PORT, '0.0.0.0', () => console.log(`Hogwarts Online is running on port ${PORT}`));
